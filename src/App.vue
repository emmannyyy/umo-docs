...
// Document loader modal visibility
const showDocumentLoader = ref(false)
...
const createNewDocument = () => {
  ...
  // Close document loader if open
  showDocumentLoader.value = false
}
...
// Show document loader modal
const openDocumentLoader = () => {
  showDocumentLoader.value = true
}
...
onMounted(() => {
  ...
  setTimeout(() => {
    if (editorRef.value) {
      ...
      // Add a test document when in development
      if (import.meta.env.DEV) {
        console.log('Adding test content in development mode')
        const testContent = `
          <h1>Test Document</h1>
          <p>This is a test document to verify that loading works.</p>
          <p>This content was added in the onMounted hook.</p>
        `
...
<template>
  ...
  <div v-if="showDocumentLoader" class="modal-overlay">
    <div class="modal-content document-loader-modal">
      <button @click="showDocumentLoader = false" class="close-button">×</button>
      <DocumentLoader @select-document="handleDocumentSelect" />
    </div>
  </div>
